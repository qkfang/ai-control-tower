...
- `deployAppServicePlan`, `deployWebApp`, `deployFoundry` — toggle which modules deploy
...
**Edit env file** [fabric_cicd/.env.dev](../fabric_cicd/.env.dev) (copy from `.env.sample` if missing). Required values:
...
| `FABRIC_ENVIRONMENT` | `DEV` / `PPE` / `PROD` (selects `.env.<env>` overlay and `parameter.yml` key) |
| `FABRIC_SUBSCRIPTION_ID` | Subscription hosting the Azure resources from step 1 |
| `FABRIC_RESOURCE_GROUP` | e.g. `rg-aictt-dev` |
...
| `FABRIC_ADLS_CONNECTION_ID` | GUID of a pre-created Fabric cloud connection bound to the ADLS account |
